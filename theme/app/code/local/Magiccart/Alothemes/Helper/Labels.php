 ...
 * @@Modify Date: 2015-06-09 11:23:21
 * @@Function:
 */
 ?>
<?php
		...
		if($showSale){
			...
			if($isSale){
				...
				if($percent){
					...
					$label = isset($config['saleText']) ? isset($config['saleText']) : $this->sale;
				}
				$html .= '<span class="sticker top-right"><span class="labelsale">' . $this->__($label) . '</span></span>';
			}
		}

		if(!$product->isSaleable()) $html .= '<span class="sticker center"><span class="labelsold">' . $this->__('Sold') . '</span></span>';
